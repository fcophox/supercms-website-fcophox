...
;
const translate = __turbopack_context__.r("[project]/node_modules/google-translate-api-x/index.cjs [app-route] (ecmascript)");
async function POST(request) {
    try {
        ...
        if (title) {
            try {
                // @ts-ignore
                const titleRes = await translate(title, {
                    to: 'en'
                });
        ...
        if (text) {
            try {
                // @ts-ignore
                const contentRes = await translate(text, {
                    to: 'en'
                });
        ...
        let translatedSlug = "";
        if (translatedTitle) {
            translatedSlug = translatedTitle.toLowerCase().trim().replace(/[^\w\s-]/g, '') // Remove non-word chars
...
//# sourceMappingURL=%5Broot-of-the-server%5D__b24f9acd._.js.map