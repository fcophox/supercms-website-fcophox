...
export default function EditServicePage(props: { params: Promise<{ id: string }> }) {
        ...
        const fetchData = async (itemId: string) => {
            ...
            if (error) {
                ...
                setModal({
                    ...
                    type: 'error',
                    title: 'Error',
                    content: `Error al cargar o servicio no encontrado: ${error.message}`
                });
            } else if (data) {
                // Load ES data
                setTitle(data.title || "");
    ...
    if (loading) {
        return <div className="p-16 text-center text-[var(--text-muted)]">Cargando servicio...</div>;
    }
        ...
        <div className="max-w-[1600px] mx-auto flex flex-col min-h-full">
            ...
            <header className="mb-8 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/services" className="text-[var(--text-muted)] flex items-center text-xl p-3 rounded-full bg-white/5 hover:bg-white/10 transition-colors w-12 h-12">
                        ←
                    </Link>
                    <div>
