...
export default function EditArticlePage(props: { params: Promise<{ id: string }> }) {
        ...
        const fetchArticle = async (articleId: string) => {
            ...
            if (error) {
                ...
                setModal({
                    ...
                    type: 'error',
                    title: 'Error',
                    content: `Error al cargar o artículo no encontrado: ${error.message}`
                });
            } else if (data) {
                // Load ES data
                setTitle(data.title || "");
                ...
                // Load Download Data (if exists in DB scheme)
                ...
                setHasDownload(!!data.download_url || !!data.download_title);
            }
            setLoading(false);
        };
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            // Upload to 'documents' bucket, fallback to 'public' if needed logic omitted for brevity
            // Assuming 'documents' bucket exists for PDFs
            ...
            try {
                const { error } = await supabase
                    ...
                    .update({
                        ...
                        // New fields (requires DB migration)
    ...
    const confirmDelete = () => {
        setModal({
            ...
            content: '¿Estás seguro de que deseas eliminar este artículo? Esta acción no se puede deshacer.'
        });
    }
    ...
    const closeModal = () => {
        if (isTranslating) return; // Prevent closing while translating
    ...
    if (loading) {
        return <div style={{ padding: "4rem", textAlign: "center", color: "var(--text-muted)" }}>Cargando artículo...</div>;
    }
            ...
            <header className="mb-8 flex justify-between items-center">
                ...
                <div className="flex gap-3">
                    <button
                        ...
                        className="bg-red-500/10 border border-red-500/20 text-red-500 w-12 h-12 rounded-full cursor-pointer transition-all flex items-center justify-center hover:bg-red-500/20"
                        disabled={isSaving}
