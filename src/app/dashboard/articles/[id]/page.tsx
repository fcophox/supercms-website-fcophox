...
export default function EditArticlePage() {
    ...
    const confirmTranslate = () => {
        setModal({
            ...
            content: 'Esto utilizará un servicio de IA para traducir el contenido del español al inglés. El contenido actual en inglés será sobrescrito. ¿Desea continuar?'
        });
    };
    ...
    const executeTranslate = async () => {
        ...
        try {
            ...
            // Check specifically for 500 which might mean API key missing
            if (response.status === 500) {
                const errorData = await response.json();
                if (errorData.error && errorData.error.includes("API key")) {
                    throw new Error("Falta la API Key de OpenAI. Configura OPENAI_API_KEY en tu archivo .env");
                }
            }
