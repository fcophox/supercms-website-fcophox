...
export default function ContactClient() {
    ...
    // Custom scroll function with "Slow start, fast arrival" (Ease In)
    const scrollToElement = (element: HTMLElement | null, offset: number = 0) => {
        if (!element) return;

        const targetY = element.getBoundingClientRect().top + window.pageYOffset - (window.innerHeight / 2) + (element.offsetHeight / 2) + offset;
        const startY = window.pageYOffset;
        const diff = targetY - startY;
        const duration = 1200; // Un poco más largo para notar la aceleración
    ...
    useEffect(() => {
        ...
        if (messageType) {
            ...
            // Iniciar scroll inmediatamente (o casi inmediatamente para asegurar renderizado)
            setTimeout(() => {
                    ...
                    <div className="relative min-h-[800px] flex items-start justify-center">
                        ...
                        <div className="relative z-10 w-full">
                            <div className="relative z-10 w-full flex flex-col gap-24">
                                ...
                                <div className="w-full max-w-6xl mx-auto" ref={selectionRef}>
                                    {/* Header Section */}
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        onClick={() => {
                                                            if (!isBlocked) {
                                                                setMessageType(type);
                                                                // The useEffect handles scrolling
                                                            }
                                                        }}
                                                        className={`group relative flex flex-col p-6 sm:p-8 min-h-[180px] md:h-[420px] rounded-[0.8rem] transition-all duration-500 border text-left h-full ${isBlocked
                                                    ...
                                                    >
                                                        {/* Radio Indicator */}
                                                        <div className={`absolute top-8 right-8 w-6 h-6 rounded-full border transition-all duration-300 flex items-center justify-center ${messageType === type
                                                        ...
                                                        <div className="mt-auto max-w-full md:max-w-[85%]">
                                                            {/* Icon container */}
                                                            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center mb-6 transition-all duration-300 ${isBlocked
                                                                ? "bg-white/5 text-white/20"
                                                                : messageType === type
                                                                    ? "bg-[#5b4eff] text-white"
                                                                    : "bg-white/5 text-[#a1a1aa] group-hover:bg-white/10 group-hover:text-white"
                                                                }`}>
                                                                {React.createElement(typeIcons[type], { size: 24 })}
                                                            </div>

                                                            <h4 className={`text-lg sm:text-xl font-light mb-2 transition-colors duration-300 leading-tight ${isBlocked
                                                                ? "text-white/40"
                                                                : messageType === type ? "text-white" : "text-white/90"
                                                                }`}>
                                                                {t(`contact.form.type.${type}`)}
                                                            </h4>

                                                            <p className={`text-xs sm:text-sm leading-relaxed transition-colors duration-300 ${isBlocked
                                                                ? "text-white/20"
                                                                : messageType === type ? "text-white/70" : "text-[#71717a]"
                                                                }`}>
                                ...
                                <AnimatePresence>
                                    {messageType && (
                                        <motion.div
                                            ...
                                            className="w-full flex flex-col items-center pb-24 border-t border-white/5 pt-24"
                                        >
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        if (selectionRef.current) {
                                                            // Use the custom scroll for consistency or native if preferred
                                                                        ...
                                                                        {calendarDays.map((date, idx) => {
                                                                            ...
                                                                            return (
                                                                                <button
                                                                                    ...
                                                                                    className={`relative w-9 h-9 mx-auto flex items-center justify-center rounded-full text-[0.9rem] transition-all duration-300 font-medium ${isSelected
                                                            ...
                                                            <div className="flex flex-col gap-4">
                                                                ...
                                                                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                                                                    {[
                                                                        ...
                                                                        // Un slot está bloqueado si:
                                                                        // 1. No se ha seleccionado una fecha aún (bloqueado por defecto)
                                                                        // 2. El día completo está restringido (restrictedDays)
                                                                        // 3. El slot específico está restringido para ese día (dailyRestrictions)
                                                                        const isDayRestricted = dayIndex === null || restrictedDays.includes(dayIndex);
                                                                        const isSlotBlocked = !formData.meetingDate || isDayRestricted || (dayIndex !== null && dailyRestrictions[dayIndex.toString()]?.includes(slot));
                                                                        ...
                                                                        return (
                                                                            <button
                                                                                ...
                                                                                className={`p-3 rounded-xl border text-[0.85rem] transition-all duration-300 whitespace-nowrap flex items-center justify-center ${isSlotBlocked
